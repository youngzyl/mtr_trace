...
echo -e "——————————————————————————————\n"
for i in {0..8}; do
	mtr -r -n -T ${iplise[i]} > $HOME/mtr_trace_temp
	grep -q "59\.43\." $HOME/mtr_trace_temp
	if [ $? == 0 ];then
		grep -q "202\.97\."  $HOME/mtr_trace_temp
		if [ $? == 0 ];then
		...
		grep -q "202\.97\."  $HOME/mtr_trace_temp
		if [ $? == 0 ];then
			grep -q "219\.158\." $HOME/mtr_trace_temp
			if [ $? == 0 ];then
			...
			echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;34m电信163\033[0m"
				...
				grep -q "218\.105\."  $HOME/mtr_trace_temp
				if [ $? == 0 ];then
				echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;35m联通9929\033[0m"
				else
					grep -q "219\.158\."  $HOME/mtr_trace_temp
					if [ $? == 0 ];then
						grep -q "219\.158\.113\." $HOME/mtr_trace_temp
						if [ $? == 0 ];then
						...
						grep -q "223\.120\."  $HOME/mtr_trace_temp
						if [ $? == 0 ];then
						echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;35m移动CMI\033[0m"
						else
							grep -q "221\.183\."  $HOME/mtr_trace_temp
							if [ $? == 0 ];then
							echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;35m移动cmi\033[0m"
							else
